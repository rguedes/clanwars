...
{
    private $source;
    private $_battleType = array(1=>"random", 13=>"cw", 10=>"sh");
            ...
            try{
                ...
                foreach ($datablockSize as $i => $value) {
                    ...
                    if (strpos($myblock,'arenaUniqueID') !== false) {
                        ...
                        $result_blocks['datablock_battle_result']['arenaUniqueID'] = sprintf("%0.0f", $result_blocks['datablock_battle_result']['arenaUniqueID']);
                        $result_blocks['identify']['arenaUniqueID'] = sprintf("%0.0f", $result_blocks['datablock_battle_result']['arenaUniqueID']);
                    }else{
    ...
    private function cleanReplayVersion($replay_version){
        $replay_version = str_replace(array(", ",' '), array(".",'.'), $replay_version);
        return $replay_version;
    }

    private function dumpjson($mydict, $filename_source, $exitcode){
        if ($exitcode == 0) {
    ...
    private function get_identify($result_blocks){
        $team = 0;
        $internaluserID = 0;
        $vehicles = $result_blocks['datablock_1']['vehicles'];

        foreach ($vehicles as $key => $value) {
            if ( $value['name'] == $result_blocks['datablock_1']['playerName'] ){
                $internaluserID = $key;
                $team = $value['team'];
                break;
            }
        ...
        $result_blocks['identify']['battleType'] = array_key_exists($result_blocks['datablock_1']['battleType'],$this->_battleType) ? $this->_battleType[$result_blocks['datablock_1']['battleType']] : $result_blocks['datablock_1']['battleType'];


        if (!array_key_exists("datablock_battle_result", $result_blocks['common']) )
            return $result_blocks;

        $players = $result_blocks['datablock_battle_result']['players'];
        foreach($players as $key => $value) {
            $value['platoonID'] = $value['prebattleID'];
            foreach ($result_blocks['datablock_battle_result']['vehicles'] as $vkey => $vvalue) {
                $vvalue = $vvalue[0];
                if ($vvalue['accountDBID'] == $key) {
                    $value['vehicleid'] = $vkey;
                    break;
                }
            }
        }
